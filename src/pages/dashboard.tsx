...
import { useRoqPlatformUserProfiles, useConversionHistoryCount, useRoleCount, useConversionCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const conversionHistoryStat = useConversionHistoryCount();

  const roleStat = useRoleCount();

  const conversionStat = useConversionCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Conversion Histories',
      result: conversionHistoryStat?.data,
      isLoading: conversionHistoryStat.isLoading,
    ...
    {
      title: 'Conversions',
      result: conversionStat?.data,
      isLoading: conversionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
