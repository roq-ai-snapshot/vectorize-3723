...
import { useRoqClient, useConversionFindFirst } from 'lib/roq';
...
function ConversionEditPage() {
  ...
  const { data, error, isLoading, mutate } = useConversionFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ConversionInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.conversion.update({
        data: values as RoqTypes.conversion,
        where: {
          id,
        },
      });
